...
namespace PetShopAPI.Auth
{
    public class UserAuthentication : IUserAuthentication
    {
        ...
        public UserAuthentication(IUserService userService, IOptions<ApiSettings> apiSettings, IPasswdHasher passwdHasher) 
        {
        ...
        public async Task<AuthenticationResponse?> Authenticate(AuthenticationRequest model)
        {
            ...
            if (userEntity!=null) 
            {
                ...
                var user = new AuthUser() { Id = userEntity.guid, Roles = roles?? Array.Empty<string>() };
